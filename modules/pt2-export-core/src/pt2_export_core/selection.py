...
a subset from data a zoo's own report already generated -- one or more ops-by-model matrices
(which operators each model uses, and in which call configurations) and per-model rows
...
Coverage is measured over every matrix the caller passes: the published dialect and any others
the zoo catalogues alongside it. The dialects a zoo reports are not nested -- an ATen graph has
`conv2d` and `scaled_dot_product_attention` where its decomposition has `convolution`, `addmm`
and `bmm` -- so a model earns its place by what it demonstrates in any of them, while what it
*costs* is always the graph that actually gets committed.
...
def build_candidates(rows, ops_by_model, *extra_ops_by_model):
    """Join parsed report rows with one or more ops-by-model matrices into
    {name: {family, nodes, ops, weight_mb, resolution}}.

    A model needs an entry in `rows` and in `ops_by_model` to be a candidate: `rows` alone
    cannot say which operators it uses, and `ops_by_model` is the matrix of the graph that
    actually gets published, so a model missing from it is not something to build a published
    artifact on. It also supplies `nodes`, the cost of committing that graph.

    `extra_ops_by_model` are further matrices -- another dialect, or the same one lowered on
    another backend -- that widen what coverage *means* without changing what a model costs.
    `ops` is the union of `op_unit()` across all of them: (operator, configuration) pairs rather
    than bare names, deduped on the configuration itself, so a pair in several matrices counts
    once even though each file numbers its configurations independently.
    """
    candidates = {}
    for name, cells in ops_by_model.items():
        ...
        for extra in extra_ops_by_model:
            ops |= {op_unit(op, config) for op, config, _ in extra.get(name, ())}
        candidates[name] = {
...
def select(candidates, target, max_nodes, max_weight_mb, include, exclude, popularity=None,
    ...
    `role_units`, if given a dict, is populated `{name: sorted([unit, ...])}` with exactly the
    (operator, configuration) units that model's own selection first covered -- the "sole/
    first coverage contributor" a role candidate is built from (see
    `role_candidates()`/`render_role_candidates()` below). A name that contributed nothing new
    (e.g. a phase-2 family representative chosen for breadth alone, once every unit it also
    happens to carry is already covered) is never given an entry.
    """
    ...
    def take(name, phase):
        contribution = candidates[name]['ops'] - covered
        selected.append((name, phase))
        covered.update(candidates[name]['ops'])
        families.add(candidates[name]['family'])
        if role_units is not None and contribution:
            role_units[name] = sorted(contribution)
...
def render_manifest(selected, candidates, target, max_nodes, max_weight_mb, release_max_weight_mb,
    ...
    lines = [
        ...
        '#',
        f'# Generated by {script} from the operator cross-references + models.md +',
        f'# model-popularity.yaml -- regenerate with `{make_target}`. Hand edits are overwritten,',
        '# except `include` and `exclude`, which are read back and honoured verbatim.',
        '#',
        ...
        '#           `include` = named by hand. Coverage spans every catalogued dialect, so a unit can',
        '#           be one an ATen graph shows (conv2d), one only its functionalized form shows',
        '#           (relu for relu_), or one only its decomposition shows (addmm).',
        '# nodes     ATen nodes in its exported graph; the cost driver for the committed JSON.',
...
# Ops interesting enough to flag as a role candidate (suggestion #6: "if models are
# intentionally included to exercise a particular behavior -- such as adaptive pooling to
# [1,1] or [7,7] -- label that role separately from a full interpreter or native-execution
# promise"). A starter list of the standard shape-edge-case ops in vision architectures --
# hand-maintained, meant to be extended as new interesting cases turn up, not a closed set.
ROLE_OP_ALLOWLIST = {
...
def role_candidates(role_units, allowlist):
    """{name: [unit, ...]} narrowed to units whose operator matches `allowlist` -- the ops
    interesting enough to flag as a possible role, not every unit a model happens to be first
    for (most first-contributions are unremarkable dtype/rank variants, not a deliberately
    exercised edge case).
    """
    result = {}
    for name, units in role_units.items():
        matched = [u for u in units if u.split(' ', 1)[0] in allowlist]
        if matched:
            result[name] = matched
    return result


def render_role_candidates(candidates, script='scripts/select_models.py'):
    """models-role-candidates.yaml: always fully regenerated by `--write-role-candidates`,
    never hand-edited, and never merged automatically into the separate, hand-maintained
    models-roles.yaml -- a human reviews this and copies in whatever is actually worth a
    label, with a real name and description, into that file.
    """
    lines = [
        '# Role candidates: models that were the sole/first contributor of a coverage unit',
        f'# matching the role allowlist in {script}. Generated by `--write-role-candidates`,',
        '# always fully regenerated -- do not edit by hand. Review and copy anything worth',
        '# labeling into the separate, hand-maintained models-roles.yaml; this file is never',
        '# merged into it automatically.',
        '',
    ]
    document = {'candidates': {name: sorted(units) for name, units in sorted(candidates.items())}}
    lines.append(yaml.safe_dump(document, sort_keys=False, default_flow_style=False, width=100))
    return '\n'.join(lines)


def load_roles(path):
    """{name: [role, ...]} from the hand-maintained models-roles.yaml, or {} if the file is
    absent (no roles curated yet)."""
    if not os.path.exists(path):
        return {}
    with open(path) as f:
        document = yaml.safe_load(f) or {}
    return {name: list(roles) for name, roles in (document.get('models') or {}).items()}
