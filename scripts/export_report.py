...
TABLE_ROW_RE = re.compile(
    ...
    r'\s*(?P<weight>[^|]*?)\s*\|\s*(?P<resolution>[^|]*?)\s*\|\s*(?P<gflops>[^|]*?)\s*\|'
...
def render_markdown(rows, timm_version, family_docs=None):
    ...
    for family in sorted(by_family):
        ...
        if doc:
            ...
            lines.append('')
        lines.append('| variant | status | params | weight (MB) | resolution | GFLOPs | dynamic (cfg) | '
                      'dynamic (export) | preprocessing | description | error |')
        lines.append('|---|---|---|---|---|---|---|---|---|---|---|')

        for row in sorted(by_family[family], key=sort_key):
            gflops = row.get('gflops')
            lines.append(
                f"| {row['name']} | {row['status']} | {row['params_str']} | "
                f"{row['weight_str']} | {row['resolution']} | {f'{gflops:.2f}' if gflops is not None else ''} | "
